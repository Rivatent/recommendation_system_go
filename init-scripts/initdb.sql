...
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
   CREATE EXTENSION IF NOT EXISTS "pgcrypto"; -- Добавляем это расширение

CREATE TABLE users (
                       id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), -- UUID в качестве первичного ключа
                       username VARCHAR(50) UNIQUE NOT NULL,
                       email VARCHAR(100) UNIQUE NOT NULL,
                       password VARCHAR(255) NOT NULL, -- Поле для хранения паролей
